...
def panel_label(ax: plt.Axes, s: str, x: float = -0.06, y: float = 1.04) -> None:
    ax.text(x, y, s.upper(), transform=ax.transAxes, fontsize=9.5,
...
def _fig1_score_capsule(ax: plt.Axes) -> None:
    ...
    ax.text(3, 78, "IPMK-HDAC3-MMP experimental biology motivates a\n"
                   "prespecified mucosal transcriptomic score.",
            fontsize=6.0, color=MUTED, va="top")

    modules = [
        ("Regulatory module", ADULT_C, "IPMK · IPPK · HDAC3 · NCOR1/2"),
...
def _fig1_evidence_landscape(ax: plt.Axes) -> None:
    ...
    ax.text(0.05, 3.68, "Adult direct endpoint cohorts", ha="center",
            fontsize=6.4, color=ADULT_C, fontweight="bold")
    ax.text(0.62, 3.68, "Adult registry\nreplication", ha="center",
            fontsize=6.0, color="#23866F", fontweight="bold", linespacing=0.95)
    ax.text(1.18, 3.68, "Pediatric / early-onset cohorts", ha="center",
    ...
    category_legend = ax.legend(handles=handles, loc="lower left", fontsize=5.2,
    ...
    size_handles = [
        plt.Line2D([0], [0], marker="o", color="none", markerfacecolor=GRAY_M,
                   markeredgecolor=WHITE, markersize=3.5, label="n = 50"),
        plt.Line2D([0], [0], marker="o", color="none", markerfacecolor=GRAY_M,
                   markeredgecolor=WHITE, markersize=5.2, label="n = 200"),
        plt.Line2D([0], [0], marker="o", color="none", markerfacecolor=GRAY_M,
                   markeredgecolor=WHITE, markersize=7.0, label="n = 1,000"),
    ]
    ax.legend(handles=size_handles, title="Bubble area", title_fontsize=5.2,
              loc="upper right", fontsize=4.8, handletextpad=0.3,
...
def _draw_distribution(
    ...
    for pos, vals, c, fc in [(0, grp0, PINJ_C, "#FAEBD7"), (1, grp1, ADULT_C, ADULT_L)]:
        ...
        # jitter dots
        seed_bytes = hashlib.sha256(f"{title}:{pos}".encode("utf-8")).digest()
        seed = int.from_bytes(seed_bytes[:4], "little")
        xs = quasirandom_x(len(vals), pos, width=0.21, seed=seed)
...
def figure2_adult_endpoint_evidence() -> None:
    ...
    def score_or_text(dataset_id: str, endpoint_name: str) -> str:
        sub = models[(models["dataset_id"] == dataset_id) & (models["endpoint_name"] == endpoint_name)]
        if sub.empty:
            return "OR not available"
        row = sub.iloc[0]
        return f"OR {float(row.effect):.2f}"
    ...
    _draw_distribution(
        ...
        n0_label="No",
        n1_label="Yes",
        or_text=score_or_text("GSE73661", "mucosal_healing"),
        ylab=True,
    )
    ...
    _draw_distribution(
        ...
        n0_label="No",
        n1_label="Yes",
        or_text=score_or_text("GSE206285", "week8_mucosal_healing"),
        ylab=False,
    )
...
def _fig2_adult_or_forest(ax: plt.Axes, models: pd.DataFrame) -> None:
    ...
    ax.set_xscale("log")
    x_max = max(2.2, max(row["hi"] for row in rows) * 1.15)
...
def figure3_age_stratified_atlas() -> None:
    ...
    fig.text(0.02, 0.99, "Endpoint-polarity clinical and mucosal-injury readout map",
             fontsize=8.5, fontweight="bold", color=INK, va="top")
...
def _fig3_evidence_matrix(ax: plt.Axes, models: pd.DataFrame) -> None:
    ...
    for row in models.itertuples():
        ...
        nudge = {
            "week8_response": -0.32,
            "week30_response": 0.32,
            "IBD_vs_control_rectum": 0.00,
            "CD_vs_control_rectum": -0.38,
            "UC_vs_control_rectum": 0.38,
            "macroscopic_inflammation_vs_normal_or_microscopic": -0.32,
            "deep_ulcer": 0.32,
            "CD_vs_nonIBD_ileum": -0.32,
            "CD_vs_nonIBD_ileum_age_sex_adjusted": 0.32,
        }.get(row.endpoint_name, 0.0)
        x = x_map[col]
        y = y_map[ds] + nudge
        color = ROLE_COLOR.get(row.clinical_role, GRAY_M)
        p05 = float(row.pvalue) < 0.05
        size = min(82, 16 + math.sqrt(float(row.n)) * 5.6)
        ...
        ax.text(x, y, f"{float(row.effect):.1f}", fontsize=4.1,
    ...
    ax.text(-0.47, y_map["GSE109142"] + 0.42, "Pediatric", color=PRED_C,
            fontsize=5.8, fontweight="bold", va="top")
    ax.text(0.01, -0.12,
            "Circle: adult or adult/mixed; square: pediatric. Filled: P < 0.05; open: P >= 0.05. Numbers inside markers are ORs.",
...
def _fig3_direction_summary(ax: plt.Axes, models: pd.DataFrame) -> None:
    ...
    ax.set_xlim(0, 100)
    ax.set_ylim(0, 100)
    ax.set_title("Endpoint context", loc="left", fontsize=7.4,
                 fontweight="bold", pad=4)
...
def _fig4_future_validation_panel(ax: plt.Axes) -> None:
    ...
    ax.add_patch(Rectangle((7, 1), 86, 12, fc=PINJ_L, ec=PINJ_C, lw=0.8,
                           alpha=0.45))
    ax.text(11, 9, "IPMK-HDAC3-MMP barrier biology is the rationale",
...
def figureS1_sensitivity_models() -> None:
    ...
    ax.set_xscale("log")
    # Keep the full adjusted-model interval visible, including the pediatric
    # phenotype estimate whose upper confidence limit exceeds the adult range.
    x_max = max(2.2, float(adj["ci_upper"].max()) * 1.25)
    ax.set_xlim(0.01, x_max)
