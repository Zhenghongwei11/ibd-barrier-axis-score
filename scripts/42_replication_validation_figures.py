...
def figure_s6_replication() -> None:
    ...
    for _, row in strata.iterrows():
        ax.text(int(row["ibd_endoseverity_num"]), row["mean"] + 0.03, f"n={int(row['n'])}", ha="center", fontsize=7, color=MUTED)
...
def figure_s7_permutation() -> None:
    ...
    for yi, r in zip(y, perm.to_dict("records")):
        ...
        # Keep the P-value clear of the observed marker when it lies beyond the null interval.
        text_x = max(hi, r["observed_or"]) * 1.12
        ax.text(text_x, yi, ptext, va="center", fontsize=8, color=INK)
    display_min = min(perm["random_or_p2_5"].min(), perm["observed_or"].min()) * 0.72
    display_max = max(perm["random_or_p97_5"].max(), perm["observed_or"].max()) * 1.45
    ax.set_xlim(display_min, display_max)
    ax.axvline(1.0, color=MUTED, linestyle="--", linewidth=0.9, zorder=0)
    ax.set_xscale("log")
    ax.set_yticks(y)
    endpoint_labels = {
        "infliximab_response": "Infliximab response",
        "week8_mucosal_healing": "Week 8 mucosal healing",
        "ibd_vs_control_one_per_participant": "IBD versus control (one biopsy per participant)",
        "wk8_endoscopic_histologic_healing": "Week 8 endoscopic and histologic healing",
        "week8_clinical_remission": "Week 8 clinical remission",
        "mucosal_healing": "Mucosal healing",
        "week30_response": "Week 30 response",
        "week8_response": "Week 8 response",
        "week6_clinical_response": "Week 6 clinical response",
    }
    labels = [f"{r['dataset_id']} · {endpoint_labels.get(r['endpoint'], str(r['endpoint']).replace('_', ' ').title())}"
              for r in perm.to_dict("records")]
    ax.set_yticklabels(labels, fontsize=7.5)
    ax.set_xlabel("Odds ratio per 1 SD (log scale; random sets matched to 13-gene structure)", fontsize=9)
    ax.set_title("Permutation benchmark: observed score vs 1,000 random feature sets of equal size", fontsize=10, pad=16)
    ax.legend(loc="upper left", bbox_to_anchor=(0.0, 1.10), ncol=2,
              fontsize=7.2, frameon=False, borderaxespad=0.0)
    ax.grid(axis="x", color=GRID, linewidth=0.6)
...
def figure_s8_localization() -> None:
    ...
    fig, axes = plt.subplots(
        1, 3, figsize=(12.5, 6.8), sharey=True,
    ...
    for ax, disease in zip(axes, diseases):
        ...
        ax.set_xticklabels(["Epithelial", "Immune", "Stromal"], fontsize=8)
        ...
        for i in range(len(genes)):
            for j in range(len(compartments)):
                ax.text(j, i, f"{det[i, j]:.1f}", ha="center", va="center", fontsize=6,
                        color="white" if mat[i, j] > vmax * 0.6 else INK)
    cax = fig.add_axes([0.925, 0.17, 0.016, 0.64])
    cbar = fig.colorbar(im, cax=cax)
    cbar.set_label("Mean log1p counts per 10,000", fontsize=8)
    cbar.ax.tick_params(labelsize=7)
    fig.suptitle(
        "Cell-type localization of score genes (GSE214695, 46,702 cells)",
        fontsize=11, color=INK, y=0.98,
    )
    fig.text(
        0.5, 0.03,
        "Heatmap color: mean log1p counts per 10,000; in-cell labels: percentage of cells with detected expression. "
        "Stromal includes endothelial, pericyte, fibroblast, myofibroblast, FRC, and glial annotations.",
        ha="center", fontsize=7.2, color=MUTED,
    )
    fig.tight_layout(rect=(0, 0.08, 0.90, 0.94))
