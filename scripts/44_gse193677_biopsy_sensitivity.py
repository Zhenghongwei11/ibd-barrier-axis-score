...
def model_row(
    ...
    formula: str,
    gee: bool = False,
    full_axis_mean: float | None = None,
    full_axis_sd: float | None = None,
) -> dict:
    base = {
    ...
    # Keep the score unit fixed across biopsy-selection subsets. Re-z-scoring
    # each subset would make the plotted odds ratios incomparable.
    if full_axis_mean is None:
        full_axis_mean = float(data["axis_score"].mean())
    if full_axis_sd is None:
        full_axis_sd = float(data["axis_score"].std(ddof=1))
    model_data["axis_score_z"] = (model_data["axis_score"] - full_axis_mean) / full_axis_sd
    try:
        if gee:
            model = smf.gee(
                formula,
...
def main() -> int:
    ...
    for scope, rule, subset, use_gee in subsets:
        rows.append(model_row(scope, rule, "unadjusted_gee" if use_gee else "unadjusted_logistic",
                              ...
                              full_axis_mean=full_axis_mean, full_axis_sd=full_axis_sd))
