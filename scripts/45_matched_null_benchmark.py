...
def odds_ratio(score: np.ndarray, y: np.ndarray, *, score_mean: float | None = None, score_sd: float | None = None) -> float:
...
def main() -> int:
    ...
    observed_score_full = score_from_gene_indices(zmat, observed_indices)
    observed_score = observed_score_full[keep_idx]
    observed_or = observed_or_from_replication()
    observed_or_recomputed = odds_ratio(
        observed_score,
        y,
        score_mean=float(np.mean(observed_score_full)),
        score_sd=float(np.std(observed_score_full, ddof=1)),
    )
    ...
    for _ in range(N_ITER):
        try:
            ...
            random_score_full = score_from_gene_indices(zmat, idx)
            score = random_score_full[keep_idx]
            random_ors.append(
                odds_ratio(
                    score,
                    y,
                    score_mean=float(np.mean(random_score_full)),
                    score_sd=float(np.std(random_score_full, ddof=1)),
                )
            )
            valid += 1
        except Exception:
